...
func (f *Function) visitInlTree(
	...
	if inlTree == nil {
		...
		if includeSelf {
			...
			return visitor(entryPc, file.PC, selfNameOff, uint32(file.Val), selfStartLine)
		}
		return nil
	}
	...
	for inlFuncIdx := range pcValueSeq {
		...
		err := visitor(pc, inlFuncIdx.PC, funcNameOff, uint32(curFile.Val), startLine)
		if err != nil {
			return err
		}
		pc = inlFuncIdx.PC
...
func (f *Function) inlines(buf []InlineFunction) ([]InlineFunction, error) {
	...
	err = f.visitInlTree(entryPc, false, /*includeSelf*/
		func(_, _ uint64, funcNameOff, fileNo, startLine uint32) error {
...
func (f *Function) inlineMapping(entryPc uint64, buf []inlineRange) ([]inlineRange, error) {
	err := f.visitInlTree(entryPc, true, /*includeSelf*/
		func(pcLo, pcHi uint64, funcNameOff uint32, fileNo uint32, _ uint32) error {
